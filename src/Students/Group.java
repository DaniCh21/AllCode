...
class Group{
    ...
    void kickStudent (int index){
        ...
        }else System.out.println("Can't kick, due to student with this index (" + index + ") does not exist.");
    }

    void kickStudent (int index, int index2){
        temp = namesOfCurrent[index];
        ...
        if (index < numberOfStudents) {
            if (numberOfKicked < namesOfKicked.length) {                            //creating an array of kicked students
                ...
                numberOfStudents--;
            } else System.out.println("This group already had kicked 30 students. Student " + temp.name + " can't be kicked.");
        }else System.out.println("Can't kick, due to student with this index (" + index + ") does not exist.");

        temp = namesOfCurrent[index2];

        if (index2 < numberOfStudents) {
            if (numberOfKicked < namesOfKicked.length) {                            //creating an array of kicked students
                numberOfKicked++;
                namesOfKicked[numberOfKicked - 1] = temp;
                System.out.println("Student " + temp.name + " kicked.");
                for (int g = 1; g < (numberOfStudents - index2 + 1); g++) {
                    namesOfCurrent[index2 - 1 + g] = namesOfCurrent[index2 + g];
                }
                numberOfStudents--;
            } else
                System.out.println("This group already had kicked 30 students. Student " + temp.name + " can't be kicked.");
        }else System.out.println("Can't kick, due to student with this index (" + index2 + ") does not exist.");
